...
<section id="@if($customBlockId){{ $customBlockId }}@else tabel @endif" class="block-tabel relative tabel-{{ $randomNumber }}-custom-padding tabel-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
    ...
    <div class="custom-styling relative z-10 px-8 py-8 lg:py-16 xl:py-20 {{ $fullScreenClass }}">
        <div class="{{ $blockClass }} mx-auto">
            ...
            @if (($button1Text) && ($button1Link))
                <div class="{{ $textClass }} buttons w-full flex flex-wrap gap-x-4 gap-y-2 mt-4 md:mt-8">
...
<style>
    .tabel-{{ $randomNumber }}-custom-padding {
    ...
    .tabel-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
