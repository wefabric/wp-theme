@php
    ...
    // Evenementen
    $eventTitleColor = $block['data']['event_title_color'] ?? '';
    $eventTextColor = $block['data']['event_text_color'] ?? '';

    $displayType = $block['data']['display_type'];
    $currentTerms = isset($_GET['event_category']) ? array_map('intval', explode(',', $_GET['event_category'])) : [];
    ...
    if ($displayType == 'show_all') {
        ...
        $events = wp_list_pluck($query->posts, 'ID');
     }
    ...
    elseif ($displayType == 'show_specific') {
        $events = $block['data']['show_specific_events'];
...
<section id="@if($customBlockId){{ $customBlockId }}@else evenementen @endif" class="block-evenementen relative evenementen-{{ $randomNumber }}-custom-padding evenementen-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
    ...
    <div class="relative z-10 px-8 py-8 lg:py-16 xl:py-20 {{ $fullScreenClass }}">
        <div class="{{ $blockClass }} mx-auto">
            ...
            @if (!empty($visibleElements) && in_array('category_filter', $visibleElements))
                @include('components.events.category-filter')
            @endif
            @if ($events)
                @include('components.events.list', ['events' => $events])
            @endif
...
<style>
    .evenementen-{{ $randomNumber }}-custom-padding {
    ...
    .evenementen-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
    ...
    .swiper-slide-duplicate .event-hidden {
            ...
            const observerCallback = (entries, observer) => {
                entries.forEach((entry, index) => {
                    if (entry.isIntersecting) {
                        ...
                        setTimeout(() => {
                            if (eventItem.classList.contains('event-hidden')) {
                                eventItem.classList.add('event-animated');
                                eventItem.classList.remove('event-hidden');
                            }
                        }, index * 200);

                        observer.unobserve(eventItem);
                    }
                });
            };
