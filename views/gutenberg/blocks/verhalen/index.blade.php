@php
    ...
    // Verhalen
    $storyTitleColor = $block['data']['story_title_color'] ?? '';
    $storyTextColor = $block['data']['story_text_color'] ?? '';

    $displayType = $block['data']['display_type'];
    $currentTerms = isset($_GET['verhaal_category']) ? array_map('intval', explode(',', $_GET['verhaal_category'])) : [];
    $multipleFilters = $block['data']['multiple_filters_enabled'] ?? false;
     ...
     if ($displayType == 'show_all') {
        ...
        if(get_post()->post_type == 'verhalen') {
            $args['post__not_in'] = [get_post()->ID];
        }

         $query = new WP_Query($args);
         $stories = wp_list_pluck($query->posts, 'ID');
     }
    ...
    elseif ($displayType == 'show_specific') {
         $stories = $block['data']['show_specific_story'];
...
<section id="@if($customBlockId){{ $customBlockId }}@else verhalen @endif" class="block-verhalen relative relative verhalen-{{ $randomNumber }}-custom-padding verhalen-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
    ...
    <div class="relative z-10 px-8 py-8 lg:py-16 xl:py-20 {{ $fullScreenClass }}">
        <div class="{{ $blockClass }} mx-auto">
            ...
            @if (!empty($visibleElements) && in_array('category_filter', $visibleElements))
                @include('components.stories.category-filter')
            @endif
            @if (!empty($visibleElements) && in_array('post_amount', $visibleElements))
                <div class="amount-text mt-2 {{ $textClass }}">{{ count($stories) }} Verhalen</div>
            @endif
            @if ($stories)
                @include('components.stories.list', ['stories' => $stories])
            @endif
...
<style>
    .verhalen-{{ $randomNumber }}-custom-padding {
    ...
    .verhalen-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
