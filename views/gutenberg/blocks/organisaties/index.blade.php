...
<section id="@if($customBlockId){{ $customBlockId }}@else{{ 'organisaties' }}@endif" class="block-organisaties relative organisaties-{{ $randomNumber }} organisaties-{{ $randomNumber }}-custom-padding organisaties-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
...
<style>
    .organisaties-{{ $randomNumber }}-custom-padding {
    ...
    .organisaties-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
    ...
    .swiper-slide-duplicate .organisation-hidden {
        animation: flyIn 0.6s ease-out forwards !important;
    }

    .organisation-animated {
            ...
            const observerCallback = (entries, observer) => {
                entries.forEach((entry, index) => {
                    if (entry.isIntersecting) {
                        const organisationItem = entry.target;

                        setTimeout(() => {
                            if (organisationItem.classList.contains('organisation-hidden')) {
                                organisationItem.classList.add('organisation-animated');
                                organisationItem.classList.remove('organisation-hidden');
                            }
                        }, index * 200);

                        observer.unobserve(organisationItem);
                    }
                });
            };
