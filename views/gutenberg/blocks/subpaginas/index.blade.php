...
<section id="@if($customBlockId){{ $customBlockId }}@else{{ 'subpaginas' }}@endif" class="block-subpaginas relative subpaginas-{{ $randomNumber }}-custom-padding subpaginas-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }}"
    ...
    <div class="layout relative z-10 px-8 py-8 lg:py-16 xl:py-20 {{ $fullScreenClass }}">
        <div class="custom-styling {{ $blockClass }} mx-auto">
            @if ($subTitle)
                <span class="subtitle block mb-2 {{ $textClass }} text-{{ $titleColor }}">{!! $subTitle !!}</span>
            @endif
            @if ($title)
                <h2 class="title mb-4 {{ $textClass }} text-{{ $titleColor }}">{!! $title !!}</h2>
            @endif
            @if ($text)
                @include('components.content', ['content' => apply_filters('the_content', $text), 'class' => 'text-' . $textColor . ' ' .  $textClass ])
            @endif
            @if($subpages)
...
<style>
    .subpaginas-{{ $randomNumber }}-custom-padding {
    ...
    .subpaginas-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
