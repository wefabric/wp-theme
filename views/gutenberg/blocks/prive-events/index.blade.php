@php
    ...
    // Privé events
    $privateEventTitleColor = $block['data']['private_event_title_color'] ?? '';
    $privateEventTextColor = $block['data']['private_event_text_color'] ?? '';

    $displayType = $block['data']['display_type'];
    $currentTerms = isset($_GET['private_event_category']) ? array_map('intval', explode(',', $_GET['private_event_category'])) : [];
    ...
    if ($displayType == 'show_all') {
        ...
        if ($currentTerms) {
            $args['tax_query'] = [
                [
                    'taxonomy' => 'private_event_categories',
                    'field' => 'id',
                    'terms' => $currentTerms,
                ],
            ];
        ...
        $privateEvents = wp_list_pluck($query->posts, 'ID');
     }
    ...
    elseif ($displayType == 'show_specific') {
        ...
        if (!empty($privateEvents)) {
            $args['post__in'] = $privateEvents;
        }
...
<section id="@if($customBlockId){{ $customBlockId }}@else prive-events @endif" class="block-prive-events relative prive-events-{{ $randomNumber }}-custom-padding prive-events-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
    ...
    <div class="relative z-10 px-8 py-8 lg:py-16 xl:py-20 {{ $fullScreenClass }}">
        <div class="{{ $blockClass }} mx-auto">
            ...
            @if (!empty($visibleElements) && in_array('category_filter', $visibleElements))
                @include('components.private-events.category-filter')
            @endif
            @if ($privateEvents)
                @include('components.private-events.list', ['privateEvents' => $privateEvents])
            @endif
...
<style>
    .prive-events-{{ $randomNumber }}-custom-padding {
    ...
    .prive-events-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
