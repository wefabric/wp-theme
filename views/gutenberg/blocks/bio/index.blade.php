@php
    ...
    $bioTitle = $block['data']['bio_title'] ?? '';
    $bioTitleColor = $block['data']['bio_title_color'] ?? '';
    $bioTextColor = $block['data']['bio_text_color'] ?? '';
    $bioBackgroundColor = $block['data']['bio_background_color'] ?? '';

    $employeeFields = get_fields($employee);
    $employeeBioText = $employeeFields['bio_text'] ?? '';
    $employeeFirstName = $employeeFields['first_name'] ?? '';
    $employeeLastName = $employeeFields['last_name'] ?? '';
    $employeeFullName = !empty($employeeFirstName) || !empty($employeeLastName) ? $employeeFirstName . ' ' . $employeeLastName : get_the_title($employee);
    $employeeImageId = $employeeFields['image'] ?? '';
    $employeeFunction = $employeeFields['function'] ?? '';
...
<section id="@if($customBlockId){{ $customBlockId }}@else bio @endif" class="block-bio block-{{ $randomNumber }} relative bio-{{ $randomNumber }}-custom-padding bio-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
            ...
            @if ($employee)
                <div class="bio-item bg-{{ $bioBackgroundColor }} rounded-{{ $borderRadius }}">
                    <div class="custom-styling px-4 py-8 sm:p-12">
                        ...
                        @endif
                        <div class="flex items-center flex-col lg:flex-row gap-y-4 gap-x-12">
                            <div class="employee-image w-full lg:w-1/5">
                            ...
                            <div class="bio-content w-full lg:w-4/5 text-{{ $bioTextColor }}">
                                @if ($employeeBioText)
                                    <div class="employee-bio-text">
                                        @include('components.content', [
                                            'content' => apply_filters('the_content', $employeeBioText),
                                            'class' => 'mb-2',
                                        ])
                                    </div>
                                @endif
                                @if ($employeeFullName)
                                    <div class="employee-full-name font-bold text-lg">
                                        {{ $employeeFullName }}
                                    </div>
                                @endif
                                @if ($employeeFunction)
...
<style>
    .bio-{{ $randomNumber }}-custom-padding {
    ...
    .bio-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
