...
<section id="@if($customBlockId){{ $customBlockId }}@else{{ 'foto-galerij' }}@endif" class="block-foto-galerij relative foto-galerij-{{ $randomNumber }}-custom-padding foto-galerij-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
...
<style>
    .foto-galerij-{{ $randomNumber }}-custom-padding {
    ...
    .foto-galerij-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
