@php
    ...
    $accordionTitleColor = $block['data']['accordion_title_color'] ?? '';
    $accordionTextColor = $block['data']['accordion_text_color'] ?? '';
...
<section id="@if($customBlockId){{ $customBlockId }}@else{{ 'accordeon' }}@endif" class="block-accordeon relative accordeon-{{ $randomNumber }}-custom-padding accordeon-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
        ...
        <div class="{{ $blockClass }} mx-auto">
            ...
            <div class="accordion" id="accordion">
                @foreach ($accordions as $index => $item)
                    <div class="accordion-item border-b @if ($flyinEffect) accordion-hidden @endif">
                        <button
                                class="accordion-header w-full flex justify-between items-center text-left py-3 font-bold text-{{ $accordionTitleColor }}"
                        ...
                        <div class="accordion-content max-h-0 overflow-hidden transition-all duration-500 ease-in-out text-{{ $accordionTextColor }}">
                            <div class="py-2">
                                {!! $item['text'] !!}
                            </div>
                        </div>
...
<style>
    .accordeon-{{ $randomNumber }}-custom-padding {
    ...
    .accordeon-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
    ...
    .swiper-slide-duplicate .accordion-hidden {
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        items.forEach(item => {
            const header = item.querySelector("[data-accordion-toggle]");
            ...
            const observerCallback = (entries, observer) => {
                entries.forEach((entry, index) => {
                    if (entry.isIntersecting) {
                        const accordionItem = entry.target;

                        setTimeout(() => {
                            if (accordionItem.classList.contains('accordion-hidden')) {
                                accordionItem.classList.add('accordion-animated');
