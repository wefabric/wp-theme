...
<section id="@if($customBlockId){{ $customBlockId }}@else{{ 'cooperaties' }}@endif" class="block-cooperaties block-{{ $randomNumber }} relative cooperaties-{{ $randomNumber }}-custom-padding cooperaties-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
    ...
    <div class="custom-styling relative z-10 px-8 py-8 lg:py-16 xl:py-20 {{ $containerClass }}">
        <div class="{{ $blockClass }} {{ $textClass }} mx-auto">
            ...
            @if ($imageId)
                <div class="relative flex flex-col md:flex-row gap-y-4 gap-x-8">
                    <div class="w-full md:w-1/2">
                        <div class="image image-{{ $randomNumber }} relative">
                            @include('components.image', [
                                'image_id' => $imageId,
                                'size' => 'full',
                                'object_fit' => 'contain',
                                'img_class' => 'w-full object-contain',
                                'alt' => $imageAlt
                            ])

                            @php
                                $cooperaties = get_posts([
                                    'post_type' => 'cooperatie',
                                    'numberposts' => -1,
                                    'post_status' => 'publish',
                                ]);
                            @endphp

                            @if ($cooperaties && count($cooperaties) > 0)
                                @foreach ($cooperaties as $cooperatie)
                                    @php
                                        $fields = get_fields($cooperatie->ID);
                                        $x = $fields['position']['x-position'] ?? null;
                                        $y = $fields['position']['y-position'] ?? null;
                                    @endphp
...
<style>
    ...
    .cooperaties-{{ $randomNumber }}-custom-padding {
    ...
    .cooperaties-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const dots = document.querySelectorAll(".cooperatie-dot");
        const infoPanel = document.getElementById("cooperatie-info-panel");
        const tooltipData = {};
