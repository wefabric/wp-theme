@php
    ...
    // Salespitch
    $salespitch = $block['data']['salespitch'] ?? '';
    $salespitchTextColor = $block['data']['salespitch_text_color'] ?? '';
    $salespitchBackgroundColor = $block['data']['salespitch_background_color'] ?? '';
...
<section id="@if($customBlockId){{ $customBlockId }}@else{{ 'salespitch' }}@endif" class="block-salespitch relative salespitch-{{ $randomNumber }}-custom-padding salespitch-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
    ...
    <div class="overflow-class">
        ...
        <div class="whitespace-class relative z-10 px-8 py-8 lg:py-16 xl:py-20 {{ $fullScreenClass }}">
            <div class="{{ $blockClass }} {{ $textClass }} mx-auto">
                ...
                @if ($salespitchBackgroundColor)
                    <div class="salespitch-background block bg-{{ $salespitchBackgroundColor }} p-8">
                @endif
                    @if ($salespitch)
                        @include('components.content', ['content' => apply_filters('the_content', $salespitch), 'class' => 'salespitch-text text-[24px] md:text-[36px] text-' . $salespitchTextColor])
                    @endif
                @if ($salespitchBackgroundColor)
                    </div>
                @endif
...
<style>
    .salespitch-{{ $randomNumber }}-custom-padding {
    ...
    .salespitch-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
