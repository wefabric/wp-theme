...
<section id="@if($customBlockId){{ $customBlockId }}@else faq @endif" class="block-faq relative faq-{{ $randomNumber }}-custom-padding faq-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
...
<style>
    .faq-{{ $randomNumber }}-custom-padding {
    ...
    .faq-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
