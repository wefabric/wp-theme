...
@if ($activities)
    <section id="@if($customBlockId){{ $customBlockId }}@else{{ 'activiteiten' }}@endif" class="block-activiteiten relative activiteiten-{{ $randomNumber }}-custom-padding activiteiten-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
...
<style>
    .activiteiten-{{ $randomNumber }}-custom-padding {
    ...
    .activiteiten-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
    ...
    .swiper-slide-duplicate .activity-hidden {
...
@if ($flyinEffect)
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const activityItems = document.querySelectorAll('.activiteit-item');
            ...
            const observerCallback = (entries, observer) => {
                entries.forEach((entry, index) => {
                    if (entry.isIntersecting) {
                        const activityItem = entry.target;

                        setTimeout(() => {
                            if (activityItem.classList.contains('activity-hidden')) {
                                activityItem.classList.add('activity-animated');
                                activityItem.classList.remove('activity-hidden');
                            }
                        }, index * 200);

                        observer.unobserve(activityItem);
                    }
                });
            };
