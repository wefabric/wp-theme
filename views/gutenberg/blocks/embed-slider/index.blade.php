...
<section id="@if($customBlockId){{ $customBlockId }}@else{{ 'embed-slider' }}@endif" class="block-embed-slider relative embed-slider{{ $randomNumber }}-custom-padding embed-slider{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
    ...
    <div class="custom-styling relative z-10 px-8 py-8 lg:py-16 xl:py-20 {{ $fullScreenClass }}">
        <div class="{{ $blockClass }} mx-auto">
            ...
            @if (is_array($embeds) && count($embeds) > 0)
                @include('components.embeds.list', ['embeds' => $embeds])
            @endif
...
<style>
    .embed-slider-{{ $randomNumber }}-custom-padding {
    ...
    .embed-slider-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
