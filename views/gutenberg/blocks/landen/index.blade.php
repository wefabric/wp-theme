@php
    ...
    $countryTitleColor = $block['data']['country_title_color'] ?? '';
    $countryTextColor = $block['data']['country_text_color'] ?? '';
...
<section id="@if($customBlockId){{ $customBlockId }}@else landen @endif" class="block-landen relative bg-{{ $backgroundColor }} {{ $customBlockClasses }}"
    ...
    <div class="relative z-10 px-8 py-8 lg:py-16 xl:py-20 {{ $fullScreenClass }}">
        <div class="{{ $blockClass }} mx-auto">
            ...
            @endif
            @if ($countries)
                @include('components.countries.list', ['countries' => $countries])
            @endif
