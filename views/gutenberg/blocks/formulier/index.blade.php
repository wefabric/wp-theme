...
<section id="@if($customBlockId){{ $customBlockId }}@else{{ 'formulier' }}@endif" class="block-formulier relative formulier-{{ $randomNumber }} formulier-{{ $randomNumber }}-custom-padding formulier-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
...
<style>
    ...
    .formulier-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
