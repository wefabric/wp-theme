...
<section id="@if($customBlockId){{ $customBlockId }}@else diensten @endif" class="block-diensten relative diensten-{{ $randomNumber }}-custom-padding diensten-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
