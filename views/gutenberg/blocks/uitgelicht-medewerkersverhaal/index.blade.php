...
$currentTerms = isset($_GET['employee_story_category']) ? array_map('intval', explode(',', $_GET['employee_story_category'])) : [];
...
if ($displayType == 'show_all') {
    ...
    if(!is_archive()){
        if(get_post()->post_type == 'employee_stories') {
            $args['post__not_in'] = [get_post()->ID];
        }
    }

    $query = new WP_Query($args);
    $employeeStories = wp_list_pluck($query->posts, 'ID');
}
...
elseif ($displayType == 'show_category') {
    ...
    $args = [
        ...
        'tax_query' => [
            [
                'taxonomy' => 'employee_story_categories',
                'field' => 'id',
...
elseif ($displayType == 'show_specific') {
    $employeeStories = $block['data']['show_specific_employee_stories'];
    if (!is_array($employeeStories) || empty($employeeStories)) {
        $employeeStories = [];
    }
    ...
    if (!empty($employeeStories)) {
        $args['post__in'] = $employeeStories;
    }
...
<section id="@if($customBlockId){{ $customBlockId }}@else{{ 'uitgelicht-medewerkersverhalen' }}@endif" class="block-uitgelicht-medewerkersverhalen relative uitgelicht-medewerkersverhalen-{{ $randomNumber }}-custom-padding uitgelicht-medewerkersverhalen-{{ $randomNumber }}-custom-margin bg-{{ $backgroundColor }} {{ $customBlockClasses }} {{ $hideBlock ? 'hidden' : '' }}"
    ...
    <div class="custom-width relative z-10 px-8 py-8 lg:py-16 xl:py-20 {{ $fullScreenClass }}">
        <div class="{{ $blockClass }} mx-auto">
            ...
            @endif
            @if ($employeeStories)
                @include('components.featured-employee-stories.list', ['employeeStories' => $employeeStories])
            @endif
...
<style>
    .uitgelicht-medewerkersverhalen-{{ $randomNumber }}-custom-padding {
    ...
    .uitgelicht-medewerkersverhalen-{{ $randomNumber }}-custom-margin {
        @media only screen and (min-width: 0px) {
    ...
    .employee-story-hidden {
        opacity: 0;
    }

    .swiper-slide-duplicate .employee-story-hidden {
        animation: flyIn 0.6s ease-out forwards !important;
    }

    .employee-story-animated {
...
@if ($flyinEffect)
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const employeeStoryItems = document.querySelectorAll('.employee-story-item');
            ...
            const observerCallback = (entries, observer) => {
                entries.forEach((entry, index) => {
                    if (entry.isIntersecting) {
                        const employeeStoryItem = entry.target;

                        setTimeout(() => {
                            if (employeeStoryItem.classList.contains('employee-story-hidden')) {
                                employeeStoryItem.classList.add('employee-story-animated');
                                employeeStoryItem.classList.remove('employee-story-hidden');
                            }
                        }, index * 200);

                        observer.unobserve(employeeStoryItem);
                    }
                });
            };

            const observer = new IntersectionObserver(observerCallback, observerOptions);
            employeeStoryItems.forEach(item => {
