...
@if($faqs)
    <div class="">
        ...
        <div class="faq-drawer lg:px-0">
			@php $id = \Illuminate\Support\Str::random(6); @endphp
            @foreach($faqs as $key => $faq)
				<div class="faq-drawer__block">
					<input class="faq-drawer__trigger" id="faq-drawer-{{$id}}-{{$key}}" type="checkbox" />
					<label class="faq-drawer__title p-6" for="faq-drawer-{{$id}}-{{$key}}">{{ $faq['question_and_answer']['question'] }}</label>
