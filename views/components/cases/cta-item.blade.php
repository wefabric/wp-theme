...
<div class="klantcase-item klantcase-cta-item group h-full w-full {{ $ctaColumnSpan == '2' ? 'sm:col-span-2' : '' }} @if ($flyinEffect) klantencase-hidden @endif">
    <div class="custom-styling h-full flex flex-col justify-center p-8 relative overflow-hidden bg-{{ $ctaBackgroundColor }}"
        ...
        <div class="klantcase-cta-content relative z-10">
            @if ($ctaSubtitle)
                <span class="klantcase-cta-subtitle block mb-2 text-{{ $ctaSubtitleColor }}">{!! $ctaSubtitle !!}</span>
            @endif
            @if ($ctaTitle)
                <h3 class="klantcase-cta-title font-bold text-lg mb-3 text-{{ $ctaTitleColor }}">{!! $ctaTitle !!}</h3>
            @endif
            @if ($ctaText)
                @include('components.content', [
                    'content' => apply_filters('the_content', $ctaText),
                    'class' => 'klantcase-cta-text mb-4 text-' . $ctaTextColor,
                ])
            @endif
