@php
    ...
    $randomId = 'vestigingenSwiper-' . $randomNumber;
...
@else
    <div class="establishment-list grid {{ $layoutClasses['mobile'] }} {{ $layoutClasses['tablet'] }} {{ $layoutClasses['desktop'] }} {{ $layoutClasses['desktop-xl'] }} gap-y-16 gap-x-4 lg:gap-x-8 py-8">
        @foreach ($establishments as $establishment)
            @include('components.establishments.list-item')
        @endforeach
    </div>
...
@if ($swiperOutContainer)
    <style>
        .vestigingenSwiper-{{ $randomNumber }} {
...
<script>
    window.addEventListener("DOMContentLoaded", (event) => {
        var establishmentSwiper = new Swiper(".{{ $randomId }}", {
            ...
            breakpoints: {
                0: {
                    loop: {{ $swiperLoop && count($establishments) > $mobileLayout ? 'true' : 'false' }},
                    slidesPerView: {{ $mobileLayout }},
                },
                640: {
                    loop: {{ $swiperLoop && count($establishments) > $tabletLayout ? 'true' : 'false' }},
                    slidesPerView: {{ $tabletLayout }},
                },
                1280: {
                    loop: {{ $swiperLoop && count($establishments) > $desktopLayout ? 'true' : 'false' }},
                    slidesPerView: {{ $desktopLayout }},
                },
                1536: {
                    loop: {{ $swiperLoop && count($establishments) > $desktopXlLayout ? 'true' : 'false' }},
