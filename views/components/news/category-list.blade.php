    ...
    @if($categories)
        @foreach($categories as $category)
            ...
            <a href="{{ $categoryLink }}"
               ...
               class="@if(empty($categoryColor) && !$isActive) border-primary hover:bg-primary hover:text-white @elseif(empty($categoryColor) && $isActive) border-primary bg-primary text-white @endif category-link text-primary hover:text-white border-2 px-4 py-2 rounded-full"
               @if(!$isActive)
