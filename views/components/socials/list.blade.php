...
@if(!empty($option) && array_key_exists('channels', $option))
    <div class="socials mb-4 flex gap-4 items-center">
        ...
        {{--        @endforeach--}}
        @foreach($option['channels'] as $social)
            <a class="group footer-social social-{{ strtolower($social['name']) }} hover:text-{{ $title_color }}" href="{{ $social['url'] }}" alt="{{ $social['name'] }}" target="_blank" aria-label="Ga naar {{ $social['name'] }}"><i
