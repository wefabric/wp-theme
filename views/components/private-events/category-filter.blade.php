@php
    ...
    $currentCatIds = isset($_GET['private_event_category']) ? array_map('intval', explode(',', $_GET['private_event_category'])) : [];
    $currentTerms = [];
    foreach ($currentCatIds as $catId) {
        $currentTerms[] = get_term($catId, 'private_event_categories');
    }
...
<div class="category-list block flex flex-wrap gap-2 {{ $textClass }}">
    @if($categories && !is_wp_error($categories))
        @php
            ...
            $allCategoriesUrl = remove_query_arg('private_event_category');
        @endphp
        ...
        @foreach($categories as $category)
            @php
                $categoryColor = get_field('category_color', 'private_event_categories_' . $category->term_id);
                $categoryIcon = get_field('category_icon', 'private_event_categories_' . $category->term_id);
                ...
                $url = empty($newCatIds) ? remove_query_arg('private_event_category') : add_query_arg('private_event_category', implode(',', $newCatIds), get_permalink());
                $style = '';
