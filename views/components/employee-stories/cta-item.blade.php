...
<div class="employee-story-item employee-story-cta-item group {{ $ctaColumnSpan == '2' ? 'sm:col-span-2' : '' }} @if ($flyinEffect) employee-story-hidden @endif">
    <div class="custom-styling h-full flex flex-col justify-center p-8 relative overflow-hidden bg-{{ $ctaBackgroundColor }}"
        ...
        <div class="employee-story-cta-content relative z-10">
            @if ($ctaSubtitle)
                <span class="employee-story-cta-subtitle block mb-2 text-{{ $ctaSubtitleColor }}">{!! $ctaSubtitle !!}</span>
            @endif
            @if ($ctaTitle)
                <h3 class="employee-story-cta-title font-bold text-lg mb-3 text-{{ $ctaTitleColor }}">{!! $ctaTitle !!}</h3>
            @endif
            @if ($ctaText)
                @include('components.content', [
                    'content' => apply_filters('the_content', $ctaText),
                    'class' => 'employee-story-cta-text mb-4 text-' . $ctaTextColor,
                ])
            @endif
