@php
    ...
    $currentCatIds = isset($_GET['testimonial_category']) ? array_map('intval', explode(',', $_GET['testimonial_category'])) : [];
    $currentTerms = [];
    foreach ($currentCatIds as $catId) {
        $currentTerms[] = get_term($catId, 'testimonial_categories');
    }
...
<div class="category-list block flex flex-wrap gap-2 {{ $titleClass }}">
    @if($categories && !is_wp_error($categories))
        ...
        @foreach($categories as $category)
            @php
                ...
                $url = empty($newCatIds) ? remove_query_arg('testimonial_category') : add_query_arg('testimonial_category', implode(',', $newCatIds), get_permalink());
                $style = '';
