...
@if($block['data']['show_slider'])
    <div class="slider event-swiper block relative">
...
@else
    <div class="event-list grid {{ $layoutClasses['mobile'] }} {{ $layoutClasses['tablet'] }} {{ $layoutClasses['desktop'] }} {{ $layoutClasses['desktop-xl'] }} gap-y-16 gap-x-4 lg:gap-x-8 py-8">
        @foreach ($events as $event)
            @include('components.events.list-item')
        @endforeach
    </div>
...
<script>
    window.addEventListener("DOMContentLoaded", (event) => {
        var eventSwiper = new Swiper(".{{ $randomId }}", {
            ...
            breakpoints: {
                0: {
                    loop: {{ $swiperLoop && count($events) > $mobileLayout ? 'true' : 'false' }},
                    slidesPerView: {{ $mobileLayout }},
                },
                640: {
                    loop: {{ $swiperLoop && count($events) > $tabletLayout ? 'true' : 'false' }},
                    slidesPerView: {{ $tabletLayout }},
                },
                1280: {
                    loop: {{ $swiperLoop && count($events) > $desktopLayout ? 'true' : 'false' }},
                    slidesPerView: {{ $desktopLayout }},
                },
                1536: {
                    loop: {{ $swiperLoop && count($events) > $desktopXlLayout ? 'true' : 'false' }},
