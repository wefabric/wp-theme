@php
    ...
    if ($logoLinkType === 'page_link') {
        ...
        $logoExternalTarget = is_array($logoLinkData) ? ($logoLinkData['target'] ?? '_blank') : '_blank';
        $logoAnchorText = is_array($logoLinkData) ? ($logoLinkData['title'] ?? '') : '';
...
<div class="logo-item group h-full @if ($flyinEffect) logo-hidden @endif">
    <div class="custom-logo-styling h-full relative rounded-{{ $borderRadius }} @if($logoUrl) group-hover:-translate-y-4 duration-300 ease-in-out @endif">
        <div class="background flex items-center justify-center w-full relative p-4 md:p-6 bg-{{ $logoBackgroundColor }}">
            @if ($logoUrl)
                <a href="{{ $logoUrl }}" @if($logoLinkType === 'external_link') target="{{ $logoExternalTarget }}" @endif
        ...
        @if (!empty($visibleElements) && in_array('name', $visibleElements) && ($logoTitle))
            @if ($logoUrl)<a href="{{ $logoUrl }}" @if($logoLinkType === 'external_link') target="{{ $logoExternalTarget }}" @endif aria-label="Ga naar {{ $logoTitle }} pagina">@endif
