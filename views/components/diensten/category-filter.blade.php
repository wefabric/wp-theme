@php
    ...
    $currentCatIds = isset($_GET['dienst_category']) ? array_map('intval', explode(',', $_GET['dienst_category'])) : [];
    $currentTerms = [];
    foreach ($currentCatIds as $catId) {
        $currentTerms[] = get_term($catId, 'dienst__categories');
    }

    $categories = get_terms([
        'taxonomy' => 'dienst__categories',
        'hide_empty' => true,
    ]);
...
<div class="category-list block flex flex-wrap gap-2 {{ $textClass }}">
    @if($categories && !is_wp_error($categories))
        @php
            ...
            $allCategoriesUrl = remove_query_arg('dienst__category');
        @endphp
        ...
        @foreach($categories as $category)
            @php
                $categoryColor = get_field('category_color', 'dienst__categories_' . $category->term_id);
                $categoryIcon = get_field('category_icon', 'dienst__categories_' . $category->term_id);
                ...
                $url = empty($newCatIds) ? remove_query_arg('dienst__category') : add_query_arg('dienst__category', implode(',', $newCatIds), get_permalink());
                $style = '';
