...
<div class="dienst-item dienst-cta-item group h-full {{ $ctaColumnSpan == '2' ? 'sm:col-span-2' : '' }} @if ($flyinEffect) dienst-hidden @endif">
    <div class="custom-styling h-full flex flex-col justify-center p-8 relative overflow-hidden bg-{{ $ctaBackgroundColor }}"
        ...
        <div class="dienst-cta-content relative z-10">
            @if ($ctaSubtitle)
                <span class="dienst-cta-subtitle block mb-2 text-{{ $ctaSubtitleColor }}">{!! $ctaSubtitle !!}</span>
            @endif
            @if ($ctaTitle)
                <h3 class="dienst-cta-title font-bold text-lg mb-3 text-{{ $ctaTitleColor }}">{!! $ctaTitle !!}</h3>
            @endif
            @if ($ctaText)
                @include('components.content', [
                    'content' => apply_filters('the_content', $ctaText),
                    'class' => 'dienst-cta-text mb-4 text-' . $ctaTextColor,
                ])
            @endif
