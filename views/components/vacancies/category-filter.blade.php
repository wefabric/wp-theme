@php
    ...
    $currentCatIds = isset($_GET['vacature_category']) ? array_map('intval', explode(',', $_GET['vacature_category'])) : [];
    $currentTerms = [];
    foreach ($currentCatIds as $catId) {
        $currentTerms[] = get_term($catId, 'vacature_categories');
    }
...
<div class="category-list block flex flex-wrap gap-2 {{ $textClass }}">
    @if($categories && !is_wp_error($categories))
        @php
            ...
            $allCategoriesUrl = remove_query_arg('vacature_category');
        @endphp
        ...
        @foreach($categories as $category)
            @php
                $categoryColor = get_field('category_color', 'vacature_categories_' . $category->term_id);
                $categoryIcon = get_field('category_icon', 'vacature_categories_' . $category->term_id);
