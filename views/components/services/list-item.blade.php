                ...
                @if (!empty($visibleElements) && in_array('category', $visibleElements))
                    <div class="service-categories absolute z-20 top-[15px] left-[15px] flex flex-wrap gap-2">
                        @foreach ($serviceCategories as $category)
                            ...
                            @endphp
                            <div style="background-color: {{ $categoryColor }}" class="service-category @if(empty($categoryColor)) bg-primary @endif text-white px-4 py-2 rounded-full flex items-center gap-x-1">
