@php
    ...
    $randomId = 'featuredEmployeeStoriesSwiper-' . $randomNumber;
@endphp
...
@if($block['data']['show_slider'])
    <div class="slider block relative">
            ...
            <div class="swiper-wrapper">
                @foreach ($employeeStories as $employeeStory)
                    <div class="swiper-slide h-auto">
                        @include('components.featured-employee-stories.list-item')
                    </div>
                @endforeach
            </div>
        ...
        <div class="swiper-navigation">
            <div class="swiper-button-next featured-employee-story-button-next-{{ $randomNumber }}"></div>
            <div class="swiper-button-prev featured-employee-story-button-prev-{{ $randomNumber }}"></div>
        </div>
    </div>
@else
    <div class="featured-employee-story-list grid {{ $layoutClasses['mobile'] }} {{ $layoutClasses['tablet'] }} {{ $layoutClasses['desktop'] }} {{ $layoutClasses['desktop-xl'] }} gap-y-16 gap-x-4 lg:gap-x-8 py-8">
        @foreach ($employeeStories as $employeeStory)
            @include('components.featured-employee-stories.list-item')
        @endforeach
    </div>
...
@if ($swiperOutContainer)
    <style>
        .featuredEmployeeStoriesSwiper-{{ $randomNumber }} {
...
<script>
    window.addEventListener("DOMContentLoaded", (event) => {
        var featuredEmployeeStoriesSwiper = new Swiper(".{{ $randomId }}", {
            spaceBetween: 20,
            ...
            @endif
            navigation: {
                nextEl: ".featured-employee-story-button-next-{{ $randomNumber }}",
                prevEl: ".featured-employee-story-button-prev-{{ $randomNumber }}",
            },
            breakpoints: {
                0: {
