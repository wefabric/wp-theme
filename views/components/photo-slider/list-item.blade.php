    ...
    @if ($imageID)
        ...
        @if ($imageLinkUrl)
            <div class="absolute w-full h-full overflow-hidden rounded-{{ $borderRadius }}">
                <a href="{{ $imageLinkUrl }}" target="{{ $imageLinkTarget }}" aria-label="Ga naar {{ $imageLinkTitle }} pagina" class="absolute w-full h-full bg-primary z-10 opacity-0 group-hover:opacity-50 transition-opacity duration-300 ease-in-out"></a>
            </div>
        @endif
