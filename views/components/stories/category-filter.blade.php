@php
    ...
    $currentCatIds = isset($_GET['verhaal_category']) ? array_map('intval', explode(',', $_GET['verhaal_category'])) : [];
    $currentTerms = [];
...
<div class="category-list block flex flex-wrap gap-2 {{ $textClass }}">
    @if($categories && !is_wp_error($categories))
        @php
            ...
            $allCategoriesUrl = remove_query_arg('verhaal_category');
        @endphp
        ...
        @foreach($categories as $category)
            @php
                ...
                $url = empty($newCatIds) ? remove_query_arg('verhaal_category') : add_query_arg('verhaal_category', implode(',', $newCatIds), get_permalink());
                $style = '';
