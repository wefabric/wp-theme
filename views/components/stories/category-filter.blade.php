@php
    ...
    $currentCatIds = isset($_GET['verhaal_category'])
        ? array_map('intval', explode(',', $_GET['verhaal_category']))
        : [];

    $currentTerms = [];
    foreach ($currentCatIds as $catId) {
        $currentTerms[] = get_term($catId, 'verhaal_categories');
    }
    ...
    // ❗ Belangrijk: gebruik de huidige URL, NIET get_permalink()
...
<div class="category-list block flex flex-wrap gap-2 {{ $textClass }}">
    @if($categories && !is_wp_error($categories))
        @php
            // URL zonder filters
            $allCategoriesUrl = remove_query_arg('verhaal_category', $currentUrl);
        @endphp
        ...
        @foreach($categories as $category)
            @php
                $categoryColor = get_field('category_color', 'verhaal_categories_' . $category->term_id);
                $categoryIcon = get_field('category_icon', 'verhaal_categories_' . $category->term_id);

                $isActive = in_array($category->term_id, $currentCatIds);

                if ($multipleFilters) {
                    $newCatIds = $isActive
                        ? array_diff($currentCatIds, [$category->term_id])
                        : array_merge($currentCatIds, [$category->term_id]);
                } else {
                    $newCatIds = $isActive ? [] : [$category->term_id];
                }

                // ❗ Bouw de URL correct met huidige pagina
                $url = empty($newCatIds)
                    ? remove_query_arg('verhaal_category', $currentUrl)
                    : add_query_arg('verhaal_category', implode(',', $newCatIds), $currentUrl);
