                ...
                <div class="absolute bottom-0 left-0 p-8 z-20">
                    @if (!empty($visibleElements) && in_array('category', $visibleElements))
                        @if ($storyCategories && !is_bool($storyCategories))
                            <div class="story-categories z-20 flex flex-wrap gap-2">
                                @foreach ($storyCategories as $category)
                                    @php
                                        $categoryColor = get_field('category_color', $category);
                                        $categoryIcon = get_field('category_icon', $category);
                                    @endphp
                                    <div style="background-color: {{ $categoryColor }}" class="story-category @if(empty($categoryColor)) bg-primary @endif text-white px-4 py-2 rounded-full flex items-center gap-x-1">
