...
<div class="header header-1 w-full @if(!empty($bg)) {{ $bg }} @endif" @if(!empty($gradient)) style="{{ $gradient }}" @endif>
    <div class="image py-15 lg:{{ $block->get('vertical_space') }} mx-4 lg:mx-20 bg-center bg-cover bg-no-repeat z-40 relative" style="background-image: url('{{ wp_get_attachment_image_url($imageId, 'full') }}')">
